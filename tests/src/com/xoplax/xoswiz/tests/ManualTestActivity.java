...
package com.xoplax.xoswiz.tests;
...
public class ManualTestActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        final Account[] accounts = mAccountManager.getAccountsByType(XOSWiz.ACCOUNT_TYPE_CMAccount);
    ...
    private void getToken() {
        mAccountManager.getAuthToken(mSelectedAccount, XOSWiz.AUTHTOKEN_TYPE_ACCESS, null, this, new AccountManagerCallback<Bundle>() {
            @Override
    ...
    private void invalidateAuthToken() {
        String token = mAccountManager.peekAuthToken(mSelectedAccount, XOSWiz.AUTHTOKEN_TYPE_ACCESS);
        if (token != null) {
            mAccountManager.invalidateAuthToken(XOSWiz.AUTHTOKEN_TYPE_ACCESS, token);
    ...
    private void expireAuthToken() {
        mAccountManager.setUserData(mSelectedAccount, XOSWiz.AUTHTOKEN_EXPIRES_IN, "0");
        String token = mAccountManager.peekAuthToken(mSelectedAccount, XOSWiz.AUTHTOKEN_TYPE_ACCESS);
        if (token != null) {
            mAccountManager.invalidateAuthToken(XOSWiz.AUTHTOKEN_TYPE_ACCESS, token);
        }
