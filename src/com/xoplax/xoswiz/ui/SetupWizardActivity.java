...
public class SetupWizardActivity extends Activity implements SetupDataCallbacks,
    ...
    private PacPagerAdapter mPagerAdapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ((InsertCoins)AppGlobals.getInitialApplication()).disableStatusBar();
        mSharedPreferences = getSharedPreferences(InsertCoins.SETTINGS_PREFERENCES, Context.MODE_PRIVATE);
        ...
        mPagerAdapter = new PacPagerAdapter(getFragmentManager());
    ...
    protected void onResume() {
        super.onResume();
        onPageTreeChanged();
       if (!InsertCoinsUtils.isNetworkConnected(this) && mTriedEnablingWifiOnce) {
            InsertCoinsUtils.tryEnablingWifi(this);
        }
    }
                    ...
                    switch (page.getId()) {
                        case R.string.setup_welcome:
                            if (!mTriedEnablingWifiOnce) {
                                ...
                                InsertCoinsUtils.launchWifiSetup(SetupWizardActivity.this);
                            }
                            break;
    ...
    private void removeUnNeededPages() {
        ...
        if (page != null && isPackageInstalled(InsertCoins.ACCOUNT_TYPE_GOOGLE, SetupWizardActivity.this) || accountExists(InsertCoins.ACCOUNT_TYPE_GOOGLE)
                || InsertCoinsUtils.isUnableToModifyAccounts(SetupWizardActivity.this)) {
            removeSetupPage(page, false);
            pagesRemoved = true;
        }
    ...
    private void doSimCheck() {
        if (!mSharedPreferences.getBoolean(KEY_SIM_MISSING_SHOWN, false)) {
            if (InsertCoinsUtils.isGSMPhone(SetupWizardActivity.this) && InsertCoinsUtils.isSimMissing(SetupWizardActivity.this)) {
    ...
    public void launchGoogleAccountSetup() {
        ...
        bundle.putBoolean(InsertCoins.EXTRA_FIRST_RUN, true);
        bundle.putBoolean(InsertCoins.EXTRA_ALLOW_SKIP, true);
        AccountManager.get(this).addAccount(InsertCoins.ACCOUNT_TYPE_GOOGLE, null, null, bundle, this, new AccountManagerCallback<Bundle>() {
            @Override
    ...
    private void finishSetup() {
        ...
        ((InsertCoins)AppGlobals.getInitialApplication()).enableStatusBar();
    ...
    private class PacPagerAdapter extends FragmentStatePagerAdapter {

        private int mCutOffPage;

        private PacPagerAdapter(FragmentManager fm) {
            super(fm);
        }
